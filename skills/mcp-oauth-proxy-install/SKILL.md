...
2. Validate that the GitHub package can be downloaded and built with `npx`/npm. This workflow intentionally follows the repository default branch so frequent changes are available immediately.
...
8. Report the repository source, config path, and the next skill to invoke.
