...
- Release: v0.3.5
- Launch command: `npx -y github:aliyoussefi/mcp-oauth-proxy#v0.3.5`
...
8. Add or replace only one Scout MCP entry in `%USERPROFILE%\.scout\m-mcp-servers.json`, preserving all unrelated entries. Use the provider name as the Scout MCP name and launch:
   ...
   ```text
   npx -y github:aliyoussefi/mcp-oauth-proxy#v0.3.5 --config "%USERPROFILE%\mcp-oauth-proxy\config.json" --server <provider-name>
   ```
